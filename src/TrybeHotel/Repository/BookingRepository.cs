...
namespace TrybeHotel.Repository
{
    public class BookingRepository : IBookingRepository
    {
        ...
        public BookingResponse GetBooking(int bookingId, string email)
        {
            ...
            var findBooking = _context.Bookings.FirstOrDefault(b => b.BookingId == bookingId) ?? throw new KeyNotFoundException("Booking not found");
            if (user.UserId != findBooking.UserId) {
                throw new Exception("This Booking doesn't belong to this user");
            }
            var response = (from booking in _context.Bookings
